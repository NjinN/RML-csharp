        ...
        public override Rtoken Run(List<Rtoken> args, Rtable ctx) {
            ...
            try {
                return (args[0].GetRtype(), args[1].tp) switch {
                    ...
                    (Rtype.Bin, Rtype.Int) => new Rtoken(Rtype.Bin, new List<byte>(LangUtil.ReverseArr(BitConverter.GetBytes(args[1].GetInt())))),
                    (Rtype.Bin, Rtype.Float) => new Rtoken(Rtype.Bin, new List<byte>(LangUtil.ReverseArr(BitConverter.GetBytes(Decimal.ToDouble(args[1].GetFloat()))))),
                    (Rtype.Bin, Rtype.Str) => new Rtoken(Rtype.Bin, new List<byte>(System.Text.Encoding.Default.GetBytes(args[1].GetStr()))),
